    ...
    <section class="contact-section section_padding">
        ...
        <script src="https://maps.googleapis.com/maps/api/js?key=YOUR_API_KEY&callback=initMap"></script>
                ...
                <div class="row">
                    ...
                    <div class="container">
                        <form name="dadosCurriculos" action="../crud/conexao.php" method="post">
                            <table class="table">
                                <tbody>
                                    <tr>
                                        <td>Nome: </td>
                                    ...
                                    <tr>
                                        <td>Objetivo: </td>
                                        <td><input type="text" name="objetivo" required></td>
                                    </tr>
                                    <tr>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao']) && $_POST['acao'] == 'inserir') {
    ...
    if ($conexao) {
        // Valida e limpa os dados recebidos do formulário
        $dados = validarDados($_POST);

        // Prepara a query SQL para inserção
        $sql = "INSERT INTO curriculos (nome, email, objetivo) VALUES (?, ?, ?)";
        
        // Prepara a declaração SQL usando Prepared Statements
        $stmt = $conexao->prepare($sql);

        // Verifica se a preparação da declaração foi bem-sucedida
        if ($stmt) {
            // Atribui os parâmetros e executa a declaração
            $stmt->bind_param("sss", $dados['nome'], $dados['email'], $dados['objetivo']);
            $stmt->execute();
            ...
            // Se houver erro na preparação da declaração
        ...
        // Se a conexão não foi estabelecida corretamente
        die("Conexão falhou: " . abrirBanco()->connect_error);
    }
} else {
    // Se não foram recebidos dados via POST ou ação não é 'inserir'
